...
% row 1: hard genotypes (arrays, already called) -> categorical -> nNIL / catiger
\node[chbox] (ch1) at (2.6,-2.0) {hard genotypes\\(SNP / dosage arrays)};
...
% row 3: competitive / ASE -> categorical (GOOGA threshold) -> GOOGA (geom) / ATLAS (rigid)
\node[chbox] (ch3) at (2.6,-4.5) {competitive counts\\(transcript, RNA / ASE)};
\node[embox] (em3) at (7.3,-4.5) {categorical $\Phi_{s,g}$\\[-1pt]{\footnotesize\color{softgray}via GOOGA threshold}};
\node[cabox] (ca3) at (12.0,-4.5) {geom $\to$ \textbf{GOOGA}\\rigid $\to$ \textbf{ATLAS}};
\draw[flow] (ch3) -- (em3);
\draw[flow] (em3) -- (ca3);
