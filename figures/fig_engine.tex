...
\node[font=\bfseries, text=ink, anchor=west] at (0.55,0.05)
   {Every caller has a coordinate here:\; (emission $\times$ duration) $+$ priors};
...
\node[pill, font=\small, align=center] at (\cRx,\rTy) {catiger\\[2pt]{\footnotesize\textcolor{softgray}{ATLAS}}};
...
% ---- right column: off the shared axes (ATLAS listed here) ----
\node[font=\footnotesize, text=ink, anchor=north west, align=left, text width=5.9cm] at (9.7,-2.05) {%
  ...
  \textbf{LB-Impute, FSFHap} external ports\\[2pt]
  \textbf{control} per-site GL, no HMM};
