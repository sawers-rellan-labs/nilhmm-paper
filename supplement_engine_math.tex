...
Text~S3 --- the $(\text{emission}\times\text{duration})$ grid; a fifth, ATLAS, wraps GOOGA
thresholding around catiger for competitive-mapping / ASE data; binhmm is an off-axis Gaussian
track; two are faithful ports of external biparental imputers, carried as baselines; and one is a
...
catiger   & categorical GT       & rigidity             & \texttt{rigidity}   & hard-call $+$ min.\ run length; the categorical-rigidity caller \\
...
ATLAS     & categorical GT       & rigidity             & GOOGA thresh        & competitive-mapping (ASE) ancestry; wraps GOOGA thresholding around catiger \\
\midrule
...
\bottomrule
\end{tabular}
\caption{The caller family. Top: the four pure $(\text{emission}\times\text{duration})$ cells of the
shared engine of Text~S3 --- \textbf{nNIL}/\textbf{catiger} (categorical confusion emission;
geometric/rigidity) and \textbf{bbNIL}/\textbf{RTIGER} (BetaBinomial; geometric/rigidity) --- plus
\textbf{ATLAS}, a caller that wraps GOOGA competitive-mapping (allele-specific-expression)
thresholding around catiger for RNA ancestry. Bottom: an off-axis Gaussian track, two external
ports, and the no-HMM control. RTIGER and TIGER \citep{rtiger_2023,tiger_2015}; LB-Impute
\citep{lbimpute_2016}.}
\label{tab:callers}
\end{table}
...
actually obey the emission model --- and only the duration remains a free choice. Hard
genotypes (arrays), and competitive/ASE counts once GOOGA-thresholded, take the categorical
emission (nNIL/catiger, and ATLAS); dosage read counts take the BetaBinomial (bbNIL/RTIGER);
binned allele fractions take a Gaussian (binhmm). The red $\times$ arrows mark the
...
is the categorical$+$rigidity caller. \textbf{ATLAS} applies it to competitive-alignment RNA data:
because expression and allele-specific expression confound the allele fraction (so the BetaBinomial
cannot model it), ATLAS thresholds the competitive counts to hard calls (GOOGA) and runs catiger ---
a wrapper caller, GOOGA $+$ catiger. \textbf{binhmm} pre-bins to $\sim$1\,Mb before a 3-state
