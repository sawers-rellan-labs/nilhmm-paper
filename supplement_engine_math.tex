...
% (engine/caller-grid schematic is now main-text Figure~1; the data-channel view is
%  Fig.~\ref{fig:dispatcher} in Text~S4.)
...
(Text~S4; main-text Fig.~1). This is the local-ancestry HMM lineage
...
$\ell$ (the RTIGER-style regularizer, \citealp{rtiger_2023,tiger_2015}), each
macro-state $m$ is expanded into a chain of
$\ell$ sub-states $S'=S\times\{1,\dots,\ell\}$: sub-states $1,\dots,\ell-1$ advance
deterministically (forcing the minimum run), and the terminal sub-state $\ell$ is free
--- it either self-loops to stay in $m$ (mass $A_{mm}$) or exits to sub-state $1$ of
another macro-state $m'$ (mass $A_{mm'}$),
\begin{equation}
T'_{(m,k)(m',k')}=
\begin{cases}
1 & m'=m,\ k'=k+1,\ k<\ell \quad\text{(forced advance)}\\
A_{mm} & m'=m,\ k=k'=\ell \quad\text{(free self-loop)}\\
A_{mm'} & m'\ne m,\ k=\ell,\ k'=1 \quad\text{(free exit)}\\
0 & \text{otherwise.}
\end{cases}
\label{eq:rigidity}
\end{equation}
All $\ell$ sub-states of a macro-state share its emission. This is a phase-type
(Erlang) sojourn; $\ell=1$ collapses Eq.~\eqref{eq:rigidity} back to
...
its current mean. Only the means are learned --- the run-length hyperparameter ($r$ or
$\ell$) is set by simulation calibration (Text~S6), not by likelihood, for the reasons
given there.
...
(Table~\ref{tab:callers}; main-text Fig.~1; the data-channel view is Fig.~\ref{fig:dispatcher}). Four are the pure cells of the shared engine of
Text~S3 --- the $(\text{emission}\times\text{duration})$ grid; on the categorical emission a
hard-call thresholding front-end splits each cell by data type, so \googa and \atlas occupy the two
categorical coordinates reached through \googa transcript thresholding (of competitive-alignment RNA)
rather than the hard calls arrays already provide (\googa geometric, a faithful port; \atlas rigidity, this
work); \binhmm is an off-axis Gaussian
track; the family-coupled \pedigree caller (Text~\ref{sec:pedigree}) adds a structural axis beyond the
grid; two are faithful ports of external biparental imputers, carried as baselines; and one is a
...
\begin{tabular}{@{}l l l l l >{\raggedright\arraybackslash}p{4.3cm}@{}}
\toprule
caller & emission & threshold & duration & key param & role \\
\midrule
\nnil      & categorical GT       & ---     & geometric            & \texttt{rrate}      & Holland's original nNIL; hard-call baseline \\
\catiger   & categorical GT       & ---     & rigidity             & \texttt{rigidity}   & hard-call $+$ min.\ run length (this work) \\
\googa     & categorical GT       & \googa   & geometric            & \texttt{rrate}      & competitive-mapping (ASE) ancestry; faithful port of Flagel's HMM \\
\atlas     & categorical GT       & \googa   & rigidity             & \texttt{rigidity}   & \googa thresholds $+$ min.\ run for sparse per-gene calls (this work) \\
\bbnil     & count / BetaBin      & ---     & geometric            & \texttt{rrate}      & low-coverage count extension; the self-transition \emph{is} the smoother \\
...
\lbimpute & external             & ---     & linkage-decay dist.\ & \texttt{recombdist} & Fragoso \emph{et al.}\ biparental imputer (ported baseline) \\
\fsfhap    & external             & ---     & family-pooled EM     & \texttt{phet}       & TASSEL parent-call $+$ 5-state EM (ported baseline) \\
\pedigree  & count or GT          & ---     & pedigree $\times$ genome BP & \texttt{design} & family-coupled belief propagation; couples relatives across the pedigree \\
\bottomrule
\end{tabular}
\caption{The caller family. Top: the $(\text{emission}\times\text{duration})$ cells of the
shared engine of Text~S3 --- \textbf{\nnil}/\textbf{\catiger} (categorical confusion emission;
geometric/rigidity) and \textbf{\bbnil}/\textbf{\rtiger} (BetaBinomial; geometric/rigidity). The
\emph{threshold} column marks the hard-call front-end that feeds the categorical emission: \nnil/\catiger
consume the hard genotype calls SNP/dosage arrays already provide (no named thresholding step), while
\textbf{\googa} and \textbf{\atlas} reach the same two categorical coordinates through \googa transcript
thresholding of competitive-alignment (allele-specific-expression) RNA --- \googa geometric (a faithful
port of Flagel's HMM), \atlas rigidity (this work). Bottom: an off-axis
Gaussian track, two external ports, and the family-coupled \textbf{\pedigree} caller (Text~\ref{sec:pedigree}).
RTIGER and TIGER \citep{rtiger_2023,tiger_2015};
\googa/\atlas \citep{flagel2019,veltsos2024}; LB-Impute \citep{lbimpute_2016}.}
\label{tab:callers}
\end{table}

\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth]{figures/dispatcher.png}
\caption{\textbf{The caller family as a data-channel dispatcher.} The observation channel (the
assay) determines the valid emission --- a specification constraint, since the counts must
actually obey the emission model --- and only the duration remains a free choice, except on the
categorical emission, where a hard-call thresholding front-end also forks by data type. Hard
genotypes (SNP arrays) take the categorical emission directly as calls (geometric/rigidity
$\to$ \nnil/\catiger); competitive/ASE transcript counts take it via \googa thresholds (geometric/rigidity
$\to$ \googa/\atlas); dosage read counts take the BetaBinomial (\bbnil/\rtiger); binned allele fractions
take a Gaussian (\binhmm). The red $\times$ arrows mark the
easily-misused routes that feed the BetaBinomial data it cannot model --- hard genotypes as
pseudo-counts, and ASE counts whose allele fraction is expression-confounded --- both into
\rtiger.}
\label{fig:dispatcher}
\end{figure}

The four shared-engine callers differ only along the two axes of Text~S3. Holding the emission
at the categorical confusion model, \textbf{\nnil} (geometric --- Holland's original) and
\textbf{\catiger} (rigidity) differ only in duration; holding it at the BetaBinomial, \textbf{\bbnil}
(geometric) and \textbf{\rtiger} (rigidity) do the same. Read across the other axis, \nnil/\bbnil share
the geometric duration and \catiger/\rtiger share rigidity, differing only in emission. \textbf{\catiger}
is the categorical$+$rigidity caller. \textbf{\googa} and \textbf{\atlas} reach the two categorical
coordinates through a different hard-call front-end, for competitive-alignment RNA / ASE data: because
expression and allele-specific expression confound the allele fraction (so the BetaBinomial cannot model
it), both threshold the competitive counts to hard calls through \googa's per-marker error-rate model
\citep{flagel2019} and then run the categorical genotype-confusion emission. \googa pairs it with the
geometric duration --- a faithful port whose Markov chain transitions on recombination fractions between
adjacent markers with no minimum-run constraint \citep{flagel2019,veltsos2024}; \atlas substitutes the
rigidity duration, a minimum-run prior suited to the sparse per-gene calls of transcript data (this work).
This \googa thresholding is a front-end on the categorical emission, not a grid axis; the dispatcher
(Fig.~\ref{fig:dispatcher}) routes each data channel to its caller. \textbf{\binhmm} pre-bins to $\sim$1\,Mb before a 3-state
Gaussian HMM, the only track that keeps the full marker density on skim data.
\textbf{\lbimpute} and \textbf{\fsfhap} are ported verbatim as external baselines ---
...
the common segment schema, so they are directly comparable. The \emph{control} baseline
is deliberately \emph{not} a member of this caller family: it is the per-site
\emph{genotype} caller \texttt{call\_gt()} of Text~S2 invoked directly (the HWE-prior
MAP, with \emph{no} transition model), separate from the ancestry callers above. It
isolates exactly what the duration prior buys, and (being het-prone at low coverage,
Eq.~\eqref{eq:hwe-ratio}) is scored by its donor-called fraction on a known
recurrent-parent region --- the ``het-excess'' baseline every ancestry caller must beat.
...
Each caller carries one free segmentation parameter --- \nnil's \texttt{rrate},
\rtiger's \texttt{rigidity}, \lbimpute's \texttt{recombdist} --- and we set it from the
...
benchmark the refined values are $\texttt{rrate}^\star\approx1.1\times10^{-3}$ (\nnil)
...
(Table~\ref{tab:noisiness}): the duration-aware HMMs \binhmm/\rtiger
are least noisy and the no-HMM \texttt{control}/\nnil noisiest.
\begin{table}[t]
\centering
...
The \rtiger caller is a Julia-free R/Rcpp port of the robust rigid-HMM (rHMM)
...
constraint, and selects the rigidity $\ell$ by an analytic minimum-distance criterion.
We reproduce the model and estimator in condensed form; the full proofs are in the
supplement of \citet{rtiger_2023}.

\subsection{The $\ell$-rigid HMM and its augmented-state equivalent}
\label{sec:rhmm-model}

A state path $\Ss=(s_1,\dots,s_T)$ is \emph{$\ell$-rigid} if every maximal run of
identical states has length $\ge \ell$; let $\Sr$ be the set of such paths. The rHMM is
a standard HMM restricted to $\Sr$,
\begin{equation}
P_{r\mathrm{HMM}}(\Oo,\Ss;\Theta_\ell):=P_{\mathrm{HMM}}(\Oo,\Ss\mid \Ss\in\Sr;\Theta)
=\prod_{t=1}^{T}\psi_{s_t}(o_t)\ \prod_{t=1}^{T}P\!\big(s_t\mid s_{t-1},\Ss\in\Sr\big),
\label{eq:rhmm}
\end{equation}
where the constrained transition is $a_{jk}$ once a run has reached length $\ell$ and is
forced to stay ($\delta_{jk}$) otherwise. The $\ell$-th-order chain is made first-order
by the same state expansion as Eq.~\eqref{eq:rigidity}: an augmented space
$S'=S\times\{1,\dots,\ell\}$ in which $(s,j)$ records a run of length $j$ in state $s$,
carrying a standard HMM $\Theta'_\ell$ with $P_{\mathrm{HMM}}(\Oo,\Ss;\Theta_\ell)=
P_{\mathrm{HMM}}(\Oo,\Ss'\mid j_T=\ell;\Theta'_\ell)$. This reduces inference from the
naive $|S|^\ell$ to $O(|S|^2 T)$.

\subsection{Rigid forward--backward (E-step)}
\label{sec:rhmm-estep}

The recursions run on $\Theta'_\ell$ and reuse the windowed emission product
\begin{equation}
\Psi^{t}_k \;=\!\!\prod_{u=\max(t-\ell+1,1)}^{\min(t,T)}\!\!\psi_k(o_u),
\label{eq:psiwindow}
\end{equation}
so that the terminal ($j=\ell$) forward mass telescopes,
\begin{equation}
\alpha'^{\,t}_{(v,\ell)}=\sum_{u\ne v}\alpha'^{\,t-\ell}_{(u,\ell)}\,a_{uv}\,\Psi^{t}_v
+\alpha'^{\,t-1}_{(v,\ell)}\,a_{vv}\,\psi_v(o_t),
\label{eq:fwd}
\end{equation}
...
The rigidity $\ell$ is chosen not by likelihood but by controlling segmentation error.
...
\begin{equation}
\mathrm{FPR}_{x\to y}(u;\Theta_\ell)=P\!\big(\Delta_{u,x,y}+C_{u,x,y}>0\big),
\qquad
\mathrm{FNR}_{x\leftarrow y}(u;\Theta_\ell)=P\!\big(\tilde\Delta_{m,x,y}\le 0\big),
\label{eq:fprfnr}
\end{equation}
from which the expected numbers of false and missed segments per sample,
$\mathrm{SE}_+(\Theta_\ell)$ and $\mathrm{SE}_-(\Theta_\ell)$, follow by a constrained
optimization over segment-length compositions. The chosen $\ell$ minimizes the total
expected segmentation error --- a \emph{minimum-distance} selection on the segment
statistics, not an MLE of $\ell$. This is the analytic counterpart of the engine's
empirical route (Text~S6): where \citet{rtiger_2023} predict the segment error from
$\Theta_\ell$ and a coverage model, nilHMM's \texttt{calibrate\_r} measures the
