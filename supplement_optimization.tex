...
Julia ($\sim$638~s/iteration, dominated by its recomputed length-$r$ emission window)
vs nilHMM C++/Rcpp ($\sim$0.05~s/iteration, flat). \textbf{Right:} each iteration's
...
population in $\sim$4~s against $\sim$22~s for the \code{hmmlearn} caller (roughly
$5\times$ faster). Peak memory separates the two: nilHMM streams one line at a time
from the memory-mapped \code{.bed} and stays near $\sim$1~GiB, whereas \code{hmmlearn}
reads the genotype matrix into memory and reaches $2.3$~GiB, the gap widening with
marker density (each measured process loads only its own marker set, so both curves
are floored only by the language runtime at the sparsest sizes).
...
\code{nNIL} is $\sim$5$\times$ faster. \textbf{Right:} peak resident memory (each
